...
def SearchInArea(area,ref, refcontour):
    """
    slides window through area
    there finds contours and compares
    contours with reference
    comparison by Hu moments

    Args:
        area (_type_): _description_
        ref (_type_): _description_
        refcontour (_type_): _description_
    """

    matchingmatrix = []
    StepSize = int(ref.shape[1]/6)
    winsize = (int(ref.shape[1]*1.2),int(ref.shape[0]*1.2))
    for (x, y, window) in SlidingWindow(area, StepSize, windowSize=(winsize[0], winsize[1])):
		# if the window does not meet our desired window size, ignore it
        if window.shape[0] != winsize[1] or window.shape[1] != winsize[0]:
            continue
        
        # contours, hierarchy = cv2.findContours(window
        #     , cv2.RETR_EXTERNAL
        #     , cv2.CHAIN_APPROX_SIMPLE)
        # for i,con in enumerate(contours):
        #     d = cv2.matchShapes(refcontour,con,cv2.CONTOURS_MATCH_I2,0)
        #     matchingmatrix.append([x,y,d,i])

        d = cv2.matchShapes(ref,window,cv2.CONTOURS_MATCH_I2,0)
        matchingmatrix.append([x,y,d])
...
def DetectObject(binary_mask, frame_number, frame):
    ...
    if frame_number > 0:
        ...
        # a = matchCont[0].reshape(matchCont[0].shape[0],1,2)
        # cv2.drawContours(RefImg, a, -1, (255,255, 0), 3)    #this version doesnt close the contour, downside:only pts
        ...
        if frame_number == 1:
            ...
            #ImageGray = binary_mask[area_contour[1]:area_contour[3],area_contour[0]:area_contour[2]]
            matchingmatrix = SearchInArea(ImageGray,RefGray,matchCont[0])
            matchingmatrix=np.array(matchingmatrix)
            mindex = np.argmin(matchingmatrix[:,[2]])
            area = [int(matchingmatrix[mindex,0]+area_contour[0]),int(matchingmatrix[mindex,1]+area_contour[1]),int(input_bbox[2]),int(input_bbox[3])]
        ...
        #find and match contour in matching area
        diff=(input_bbox[0]-area[0], input_bbox[1]-area[1])       #drift
        PrevShiftedMatch = matchCont[0]-diff                    #drift correction
        PrevShiftedMatch = PrevShiftedMatch.reshape(PrevShiftedMatch.shape[0],1,2)
        bb_contours = FindContoursinBbox((area[0], area[1], RefImg.shape[1], RefImg.shape[0]), contours)
        matchCont = FindMatchingContour(bb_contours, PrevShiftedMatch, frame, tol, .7)
        M = cv2.moments(matchCont[0])
        cx = int(M['m10']/M['m00'])
        cy = int(M['m01']/M['m00'])       
        
        return matchCont[0], (cx,cy), (area[0], area[1], RefImg.shape[1], RefImg.shape[0])
...
if __name__ == "__main__":
    ...
    This approach uses a Hu moments comparison in sliding windows
        curiously is matching really bad....!
    """
